...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8218159061:AAEnTt7cPGYHLSwXzOQhkb1xMsWWXr00jQM"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "8335116442"))
...
BRAND       = "ѕιмяαη нoѕтιηg ᴅᴇsᴛʀᴏʏᴇʀ"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@DESTROYER_BHAI"
UPDATE_CH   = "https://t.me/DESTROYER_LIKES_GROUP"
...
def cmd_start(m: types.Message) -> None:
    ...
    action = parts[1]  # accept ya reject
    token = parts[2]

    pending = get_setting("pending_uploads", {}) or {}
    item = pending.get(token)

    if not item:
        bot.answer_callback_query(call.id, "⚠️ Ye upload request expire ya delete ho chuki hai.", show_alert=True)
        try:
            bot.edit_message_text("❌ Request already processed or not found.", call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        return
    ...
    if action == "accept":
        # Request approved - ab file download karke user ke bots me deploy/save karein
        try:
            file_info = bot.get_file(file_id)
            downloaded_file = bot.download_file(file_info.file_path)
            
            # Safe path par save karein
            user_bot_dir = DIRS["bots"] / str(target_uid) / safe_name(file_name.rsplit('.', 1)[0])
            user_bot_dir.mkdir(parents=True, exist_ok=True)
            
            file_path = user_bot_dir / file_name
            file_path.write_bytes(downloaded_file)
            
            # Pending se remove karein
            pending.pop(token, None)
            set_setting("pending_uploads", pending)

            # Admin message update karein
            bot.edit_message_text(
                f"<b>{G['ok']} Aᴘᴘʀᴏᴠᴇᴅ & Sᴀᴠᴇᴅ</b>\n\n"
            ...
            try:
                bot.send_message(
                    target_uid,
                    f"<b>{G['ok']} Uᴘʟᴏᴀᴅ Aᴘᴘʀᴏᴠᴇᴅ!</b>\n\n"
                    f"Aapki file <code>{esc(file_name)}</code> successfully approve aur deploy ho chuki hai!"
                )
            except Exception:
                pass
        ...
        try:
            bot.send_message(
                target_uid,
                f"<b>{G['no']} Uᴘʟᴏᴀᴅ Rᴇᴊᴇᴄᴛᴇᴅ</b>\n\n"
                f"Aapki file <code>{esc(file_name)}</code> admin dwara reject kar di gayi hai."
            )
        except Exception:
            pass
...
@bot.message_handler(content_types=['document'])
def handle_user_file_upload(message):
    uid = message.from_user.id
    
    # Agar user banned hai toh rok dein
    if db_load_ro().get("users", {}).str(uid, {}).get("banned"):
        bot.reply_to(message, "⚠️ Aap ban hain, file upload nahi kar sakte.")
        return

    doc = message.document
    file_name = doc.file_name or "bot_file.py"
    file_id = doc.file_id

    # Check karein ki approval required hai ya nahi
    appr_on = bool(get_setting("approval_required", True))
    
    if not appr_on and is_admin(uid):
        # Agar admin hai aur approval off hai toh direct process karein
        bot.reply_to(message, "✅ Admin upload: Direct processing...")
        return

    # File ko pending dict me save karein
    pending = get_setting("pending_uploads", {}) or {}
    upload_token = rand_token(8)
    
    pending[upload_token] = {
        "uid": uid,
    ...
    # User ko notification dein
    bot.reply_to(
        message,
        f"<b>{G['ok']} Fɪʟᴇ Rᴇᴄᴇɪᴠᴇᴅ!</b>\n\n"
        f"Aapki file <code>{esc(file_name)}</code> admin ke paas approval ke liye bhej di gayi hai. "
        f"Review hone ke baad aapko notify kar diya jayega."
    )

    # Admin/Owner ko interactive buttons ke sath notify karein
    admin_text = (
        f"<b>{G['warn']} Nᴇᴡ Fɪʟᴇ Uᴘʟᴏᴀᴅ Pᴇɴᴅɪɴɢ</b>\n\n"
        f"• <b>User:</b> <code>{uid}</code> (@{esc(message.from_user.username or 'none')})\n"
        f"• <b>File:</b> <code>{esc(file_name)}</code>\n"
        f"• <b>Token:</b> <code>{upload_token}</code>"
    )
    
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("✅ Aᴘᴘʀᴏᴠᴇ", callback_data=f"appr_accept_{upload_token}", style="success"),
        Btn("❌ Rᴇᴊᴇᴄᴛ",  callback_data=f"appr_reject_{upload_token}", style="danger")
    )

    # OWNER_ID ya sabhi admins ko bhejein
    try:
        bot.send_message(OWNER_ID, admin_text, parse_mode="HTML", reply_markup=kb)
    except Exception as e:
        print(f"[upload] Admin notification failed: {e}")


@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("appr_"))
